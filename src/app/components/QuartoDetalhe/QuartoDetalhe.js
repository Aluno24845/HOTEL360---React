...
export default function QuartoDetalheView() {
    ...
    return <div key={id} className="group" style={{ display: 'flex', flexDirection: 'column', }}>
        ...
        <h2 className="text-5xl font-bold tracking-tight text-gray-900 sm:text-1xl" style={{ marginBottom: 20 }}>Quarto - {nome}</h2>
        <div style={{ display: 'flex', flexDirection: 'column', padding: '8px' }}>
            ...
            <p className="mt-1 text-lg font-medium text-gray-900">Capacidade:</p>
            <p className="mt-1 text-lg text-gray-900">{capacidade}</p>
            <p className="mt-1 text-lg font-medium text-gray-900">Descrição:</p>
            <p className="mt-1 text-lg text-gray-900">{descricao}</p>
            <p className="mt-1 text-lg font-medium text-gray-900">Localização:</p>
            <p className="mt-1 text-lg text-gray-900">{localizacao}</p>
        </div>
        {context.utilizador && context.utilizador.role === 'Gerentes' &&
            <div className="text-white font-semibold bg-indigo-500 shadow-sm" style={{ display: 'flex', justifyContent: 'space-around', padding: '3px' }}>
                <a style={{ alignItems: 'center', borderRight: '1px solid white', flexGrow: 1, justifyContent: 'center' }} className="flex" href={`/quartos/${id}`}><HiPencil />Editar/Apagar</a>
            </div>
        }
    </div>
}